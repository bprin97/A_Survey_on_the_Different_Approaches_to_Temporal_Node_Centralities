...
// Write the TopK results
void writeResult(std::vector<std::pair<tglib::NodeId, double>> top_k, std::string filename)
{
	// Create the File
	try
	{
...
int main (int argc, char** argv)
{
    ...
    try {
        ...
        tglib::TopkResult<double> res(10);
	    for(size_t i=0; i<br.strictShortest.size(); i++)
	    {
		    // Create the Top K Results vector
            res.insert(i+1,br.strictShortest[i]);
	    }
        // Retrive the Top K Results found at the previous step
		
	    std::vector<std::pair<tglib::NodeId, double>> top_k_results =  res.getResults();
		if (vm.count("name_file_results"))
        {
			if(std::filesystem::path(vm["name_file_results"].as<std::string>()).extension() == ".txt")
            {
				// Save to file
		        writeResult(top_k_results , vm["name_file_results"].as<std::string>());
			}else
            {
