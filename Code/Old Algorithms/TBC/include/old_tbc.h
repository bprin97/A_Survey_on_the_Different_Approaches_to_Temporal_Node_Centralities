#ifndef BC_COMPUTE_OLD_TBC_H
#define BC_COMPUTE_OLD_TBC_H
// Libraries:
    ...
    public:
        ...
        */
        ...
        TemporalBetweennessCetrality(tglib::OrderedEdgeList<E> const &TEMPORAL_GRAPH_, int COST_FUNCTION ,int delta = -1) : 
            TEMPORAL_GRAPH  ( TEMPORAL_GRAPH_ ),
            EDGES           ( TEMPORAL_GRAPH.getEdges() ),
            N               ( TEMPORAL_GRAPH.getNumberOfNodes() ),
            TI_FIRST        ( TEMPORAL_GRAPH.getTimeInterval().first ),
            TI_SECOND       ( TEMPORAL_GRAPH.getTimeInterval().second ),
            T               ( this->TI_SECOND - this->TI_FIRST ),
        ...
        {   
            ...
            // 3 - Costs computed during Dijkstra
            costs.resize(VerticesApperenceSet_SIZE);
            best_costs.resize(VerticesApperenceSet_SIZE);
            best_cost_tn.resize(VerticesApperenceSet_SIZE);

            // 4 - Number of temporal walks from node s to node v
            sigma_s_v.resize(N);
            // 5 - Number of temporal walks from s to vertex apperence v_t
            sigma_s_vt.resize(N);
            // 6 - Temporal pair dependecies on s and z for each v_t
            temporal_pair_dependecies.resize(N);

            std::cout << "[DEBUG] INITIALIZATION DONE" << std::endl;
            std::cout << "[DEBUG] \t---> MEMORY USED BY NOW: " << (memory/10e9) <<  " GB" << std::endl;
            //-----------------------------------------------------------------------
            ...
            for(tglib::NodeId n_id = 0; n_id < N; n_id++)
            {
                ...
                std::cout << "[DEBUG] \tCOMPUTATION PREDECESSOR GRAPH OF NODE " << n_id+1 << "/" << N << std::endl;
                StaticGraph<int> sg = predecessor_graphs[n_id];
                memory += sizeof(int) * sg.getNodes().size() + sizeof(std::set<int>);
                memory += sizeof(std::map<int, std::set<int>>);
                for(const auto& [key, value] : sg.getEdges()){
                    memory += sizeof(int) + sizeof(int)*value.size() + sizeof(std::set<int>);
                }
                memory += sizeof(std::map<int, std::set<int>>);
                for(const auto& [key, value] : sg.getTranspose()){
                    memory += sizeof(int) + sizeof(int)*value.size() + sizeof(std::set<int>);
                }
                std::cout << "[DEBUG] \t---> MEMORY USED BY NOW: " << (memory/10e9) <<  " GB" << std::endl;
            }

            std::cout << "[DEBUG] PREDECESSORS GRAPHS COMPUTATION DONE" << std::endl;
    ...
    private:
        ...
        */
        ...
        void computePredecessorGraph(tglib::NodeId source_node)
        {
            ...
            for(int node = 0; node < N; node++){
                int best_tn = best_cost_tn[source_node][node];
                if(best_tn == -1){
                    continue;
                }
                buildOptimalPredecessorGraph(best_tn, source_node, predecessor_graphs[source_node]);
            }
        }
        ...
        */
        ...
        void initializeDijkstra(tglib::NodeId source_node){
            ...
            costs[source_node].resize(VerticesApperenceSet_SIZE, INF);
            best_costs[source_node].resize(N, INF);
            // Set to "void" the TimeNode with the best cost
            best_cost_tn[source_node].resize(N, -1);
            ...
            for(int tn_idx = 0; tn_idx < VerticesApperenceSet_SIZE; tn_idx++){
                ...
                if(source_node == getTimeNode(tn_idx).first){
                    // Set cost to zero
                    costs[source_node][tn_idx] = 0;
                }
                // Push the source node into the priority queue
                priority_queue.push(cost_idx_pair{costs[source_node][tn_idx], tn_idx});
            }
            best_costs[source_node][source_node] = 0;                      
        }

        void Dijkstra(tglib::NodeId source_node){
            ...
            while(!priority_queue.empty()){
                ...
                for(tglib::Time t : VerticesApperence_SET[v_id]){
                    ...
                    for(int wt_idx : static_edges[vt_idx]){
                        ...
                        if(costs[source_node][wt_idx] > costs[source_node][vt_idx] + getCost(v_t, w_t)){
                            ...
                            costs[source_node][wt_idx] = costs[source_node][vt_idx] + getCost(v_t, w_t);

                            // Add to the priority_queue the pair (cost, w_id); w_id = ID of node w
                            priority_queue.push(cost_idx_pair{costs[source_node][wt_idx], wt_idx});
                        ...
                        if(best_costs[source_node][w_id] > costs[source_node][wt_idx]){
                            best_costs[source_node][w_id] = costs[source_node][wt_idx];
                            best_cost_tn[source_node][w_id] = wt_idx;
                        }
                    }
                }
            }
       ...
       void generalBetweenness() {
            // Initialize the number of temporal walks from each node to all the other nodes of the graph
            for(tglib::NodeId node_id = 0; node_id < N; node_id++){
                countWalk(node_id);
            }
            
            // Initialize all betweeness centrality scores to 1
            betweeness.resize(N, 1);

            // Compute the betweeness centrality for each node of the graph
            // Line 4 of Algorithm 3 from the paper
            for(tglib::NodeId node_id = 0; node_id < N; node_id++){
                ...
                std::map<int, std::set<int>> edges = predecessor_graphs[node_id].getEdges();
                ...
                std::map<int, std::set<int>> transpose = predecessor_graphs[node_id].getTranspose();
                ...
                for(const auto &[head_idx, NOT_USED_VARIABLE] : edges){
                    ...
                    if(temporal_pair_dependecies[node_id][head_id][head_idx] != INF){
                        temporal_cumulative_dependecies[head_idx] += temporal_pair_dependecies[node_id][head_id][head_idx];
                    }
                    ...
                    for(int tail_idx : predecessors_set){
                        ...
                        double fraction_sig_vt_by_sig_wt = static_cast<double>(sigma_s_v[node_id][tail_id])/sigma_s_v[node_id][head_id];
                        ...
                        betweeness[node_id] += fraction_sig_vt_by_sig_wt * temporal_cumulative_dependecies[head_idx];
                    }
                }
                // Line 12 of Algorithm 3 from the paper 
                for(int tn_idx : predecessor_graphs[node_id].getNodes()){
                    tglib::NodeId tn_id = getTimeNode(tn_idx).first;
                    if(temporal_pair_dependecies[node_id][tn_id][tn_idx] > 0){
                        betweeness[node_id] -= 1;
                    }
                }
            }
        }
        ...
        void countWalk(tglib::NodeId source_node){            
            ...
            sigma_s_v[source_node].resize(N, 0);
            // 2 - Number of temporal walks from source_node to vertex apperence v_t
            sigma_s_vt[source_node].resize(VerticesApperenceSet_SIZE, 0);
            // 3 - Temporal pair dependecies on source_node and z for each v_t
            temporal_pair_dependecies[source_node].resize(N, std::vector<double>(VerticesApperenceSet_SIZE, 0));
            ...
            for(std::vector<int> &SCC: SCCs){
                ...
                for(int &tn_idx : SCC){
                    costs[source_node][tn_idx] = INF;
                    sigma_s_vt[source_node][tn_idx] = INF;
                    predecessor_graphs[source_node].removeNode(tn_idx);
                }
            }

            // Now, the predecessor graph is a DAG

            std::map<int, std::set<int>> edges = predecessor_graphs[source_node].getEdges();
            ...
            for(tglib::NodeId node_id; node_id < N; node_id++){
                for(int tn_idx : predecessor_graphs[node_id].getNodes()){
                    ...
                    if(costs[source_node][tn_idx] == best_costs[source_node][node_id]){
                        sigma_s_v[source_node][node_id] += sigma_s_vt[source_node][tn_idx]; 
                    }
                }
            }
            ...
            for(int tn_idx = 0; tn_idx < VerticesApperenceSet_SIZE; tn_idx++){
                ...
                int sigma_s_node = sigma_s_v[source_node][node_id];
                ...
                if(sigma_s_node == 0 || sigma_s_node == INF){
                    ...
                    temporal_pair_dependecies[source_node][node_id][tn_idx] = 0;
                }
                else{
                    // Otherwise, set the temporal pair dependecies of s,v,(v,t) to 
                    // ( #paths from s to (v,t) ) / ( #paths from s to v )
                    int sigma_s_timenode = sigma_s_vt[source_node][tn_idx];
                    double delta = static_cast<double>(sigma_s_timenode)/sigma_s_node;
                    temporal_pair_dependecies[source_node][node_id][tn_idx] = delta;
                }
            }
        }
        ...
        */
        ...
        std::vector<std::vector<int>> Kosaraju(tglib::NodeId source_node)
        {
	        ...
	        std::set<int> visited;
            std::set<int> nodes = predecessor_graphs[source_node].getNodes();
            std::map<int, std::set<int>> edges = predecessor_graphs[source_node].getEdges();
            ...
            std::map<int, std::set<int>> transpose_list = predecessor_graphs[source_node].getTranspose();
        ...
        */
        ...
        std::map<tglib::NodeId, StaticGraph<int>> predecessor_graphs;
        ...
        std::vector<std::vector<double>> best_costs;
        // Corresponding index to the TimeNode with the best cost
        std::vector<std::vector<int>> best_cost_tn;
        ...
        std::vector<std::vector<int>> sigma_s_v;
        // Number of temporal walks from s to vertex apperence v_t
        std::vector<std::vector<int>> sigma_s_vt;
        // Temporal pair dependecies on s and z for each v_t
        std::vector<std::vector<std::vector<double>>> temporal_pair_dependecies;
